...
class AssetManager extends Component {
  ...
  _deleteAsset(asset) {
    ...
    if (confirm) {
      Req("/admin/asset", "DELETE", { asset: asset.name }).then(res => {
        if (res.status === 200) {
          this._getAssets();
  ...
  _onDrop(files) {
    if (!this.state.isUploading) {
      this.setState({ isUploading: true }, () => {
        files.forEach(f => {
          Req("/admin/asset", "POST", f, "formData")
            .then(res => {
              if (res.status === 200) {
                this._getAssets();
      ...
      <>
        <div className="asset-actions">
          <label>
            ...
            />
            <button type="button" onClick={() => this._clearSearch()}>
              Clear
            </button>
          </label>
          <label>Sort by:</label>
        ...
        <DropZone onDrop={files => this._onDrop(files)} className="drop-zone">
