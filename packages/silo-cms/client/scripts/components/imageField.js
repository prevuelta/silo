...
const File = props => {
  ...
  return (
    <div className={isSelected ? "file-display selected" : "file-display"}>
      <div className="image">
        <img onClick={selectFile} src={`/images/thumbs/${f.filename}`} />
      </div>
...
class ImageField extends Component {
  ...
  render() {
    ...
    const noResult = searchStr ? `No results for '${searchStr}'` : "No files";
    return (
      <div className="field-image">
        <div className="preview">
          <div className="image">
            {src ? (
              <img
                src={`${clientSettings.fileEndpoint}/thumbs/${src}`}
                alt=""
                height="100"
                title=""
              />
          ...
          <input data-type="image" type="text" value={src || ""} readOnly />
        ...
        >
          ...
          <DropZone onDrop={this.onDrop.bind(this)} className="drop-zone">
            {isUploading ? <MiniLoader /> : <p>Upload new image/s</p>}
          </DropZone>
          {!isUploading && (
            <button onClick={this.hideModal.bind(this)}>Done</button>
          )}
        </Modal>
      </div>
    );
